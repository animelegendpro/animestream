...
<section class="product-page spad">
        <div class="container">
            <div class="row">
                <div class="col-12">
                    <h2 class="text-white mb-4">{{ ucwords($displayGenre) }} Anime</h2>
                </div>
            </div>
                    ...
                    <nav aria-label="Page navigation">
                        <ul class="pagination justify-content-center">
                            @if($totalPages > 1)
                                @if($currentPage > 1)
                                    <li class="page-item">
                                        <a class="page-link" href="{{ route('anime.genre', ['genre' => $genre, 'page' => $currentPage - 1]) }}">
                                ...
                                @for($i = max(1, $currentPage - 2); $i <= min($totalPages, $currentPage + 2); $i++)
                                    <li class="page-item {{ $i == $currentPage ? 'active' : '' }}">
                                        <a class="page-link" href="{{ route('anime.genre', ['genre' => $genre, 'page' => $i]) }}">
                                            {{ $i }}
                                        </a>
                                    </li>
                                @endfor
